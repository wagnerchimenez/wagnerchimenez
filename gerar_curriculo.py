#!/usr/bin/env python3
"""Gera currículo Word (artifacts/download) e PDF (raiz do repositório)."""
...
DOCX_OUT = ARTIFACTS / "Curriculo_Wagner_Lima_Chimenez.docx"
PDF_OUT = ROOT / "Curriculo_Wagner_Lima_Chimenez.pdf"
FONT_REG = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
FONT_BOLD = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"

RESUMO = (
    "Desenvolvedor Full Stack Sênior com ~15 anos em aplicações web, foco em PHP 8, "
    "Laravel e React (Inertia). Experiência em e-commerce de médio/grande porte (Tray), "
    "SaaS próprios em produção, APIs REST, filas (RabbitMQ), Redis, Docker, TDD e "
    "monitoramento (New Relic/Graylog). Também atuei com Symfony em sistemas corporativos. "
...
def add_job_docx(doc, title, meta, bullets):
    ...
    for b in bullets:
        ...
        set_run_font(bp.add_run(b), size=9.5)


def add_product_docx(doc, name, url, bullets):
...
def build_docx():
    ...
    add_paragraph(
        doc,
        "Dourados, MS – Brasil | Remoto | wagnerllchimenez.comp@gmail.com",
        size=9,
        space_after=1,
        align=WD_ALIGN_PARAGRAPH.CENTER,
    ...
    add_paragraph(doc, RESUMO, size=9.5, space_after=4)

    add_heading_line(doc, "Produtos e projetos em produção")
    for name, url, bullets in PRODUCTS:
        add_product_docx(doc, name, url, bullets)
    ...
    for title, meta, bullets in JOBS:
        add_job_docx(doc, title, meta, bullets)
    ...
    for c in CERTS:
        ...
        bp.paragraph_format.space_before = Pt(0)
        bp.paragraph_format.space_after = Pt(1)
        set_run_font(bp.add_run(c), size=9.5)

    doc.save(DOCX_OUT)
    print(f"DOCX (download): {DOCX_OUT}")
...
def pdf_heading(pdf: FPDF, text: str):
    pdf.ln(3)
...
def pdf_bullet(pdf: FPDF, text: str, size=9.5):
    pdf.set_x(pdf.l_margin)
...
def build_pdf():
    ...
    pdf.set_text_color(26, 58, 92)
    pdf.cell(0, 8, "Wagner Lima Chimenez", align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("DejaVu", "B", 11)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(
        0,
    ...
    pdf_heading(pdf, "Resumo")
    pdf.set_font("DejaVu", size=9.5)
    pdf.set_text_color(0, 0, 0)
    ensure_left()
    pdf.multi_cell(0, 4.5, RESUMO)

    pdf_heading(pdf, "Produtos e projetos em produção")
    for name, url, bullets in PRODUCTS:
        ensure_left()
        ...
        ensure_left()
        pdf.set_font("DejaVu", size=9)
        pdf.set_text_color(26, 92, 150)
        pdf.multi_cell(0, 4.5, url)
        pdf.set_text_color(0, 0, 0)
        for b in bullets:
            ensure_left()
            pdf_bullet(pdf, b)
    ...
    pdf_heading(pdf, "Competências técnicas")
    for label, value in SKILLS:
        ensure_left()
        pdf.set_font("DejaVu", "B", 9.5)
        pdf.multi_cell(0, 4.5, f"{label}{value}")

    pdf_heading(pdf, "Experiência profissional")
    for title, meta, bullets in JOBS:
        ensure_left()
        pdf.set_font("DejaVu", "B", 10)
    ...
    pdf_heading(pdf, "Formação")
    ensure_left()
    pdf.set_font("DejaVu", size=9.5)
    pdf.multi_cell(0, 4.5, "Pós-graduação em Gestão de TI — Anhanguera Educacional · 2013–2014")
    ensure_left()
    pdf.multi_cell(0, 4.5, "Bacharelado em Ciência da Computação — UNIGRAN · 2005–2010")

    pdf_heading(pdf, "Certificações relevantes")
    for c in CERTS:
        ensure_left()
        pdf_bullet(pdf, c)

    pdf.output(str(PDF_OUT))
    print(f"PDF (repo): {PDF_OUT}")
